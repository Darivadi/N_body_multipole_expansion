...
RETURN: 0
****************************************************************************************************/
int pc_force_monopole(int m, int n)
{
  double aux_mass, aux_factor, dist, aux_x, aux_y, aux_z, aux_pos[3];
  double xc, yc, zc;  
...
RETURN: 0
****************************************************************************************************/
int pc_force_mono_dip(int m, int n)
{
  double aux_mass, aux_factor, dist, aux_x, aux_y, aux_z, aux_pos[3];
  double xc, yc, zc;
  ...
  */
  ...
  aux_factor1    = gp[n].mass_pos[X] - aux_pos[X]*gp[n].mass;
  part[m].acc[X] += aux_factor * aux_factor1;
  part[m].dipole[X] += aux_factor * aux_factor1;

  aux_factor1    = gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass;
  part[m].acc[X] += -3.0 * aux_pos[X] * aux_pos[Y] * dist5 * aux_factor1;
  part[m].dipole[X] += -3.0 * aux_pos[X] * aux_pos[Y] * dist5 * aux_factor1;
  
  aux_factor1    = gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass;
  part[m].acc[X] += -3.0 * aux_pos[X] * aux_pos[Z] * dist5 * aux_factor1;
  part[m].dipole[X] += -3.0 * aux_pos[X] * aux_pos[Z] * dist5 * aux_factor1;
  ...
  */
  ...
  aux_factor1    = gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass;
  part[m].acc[Y] += aux_factor * aux_factor1;
  part[m].dipole[Y] += aux_factor * aux_factor1;

  aux_factor1    = gp[n].mass_pos[X] - aux_pos[X]*gp[n].mass;
  part[m].acc[Y] += -3.0 * aux_pos[X] * aux_pos[Y] * dist5 * aux_factor1;
  part[m].dipole[Y] += -3.0 * aux_pos[X] * aux_pos[Y] * dist5 * aux_factor1;

  aux_factor1    = gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass;
  part[m].acc[Y] += -3.0 * aux_pos[Y] * aux_pos[Z] * dist5 * aux_factor1;
  part[m].dipole[Y] += -3.0 * aux_pos[Y] * aux_pos[Z] * dist5 * aux_factor1;
  ...
  */
  ...
  aux_factor1    = gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass;
  part[m].acc[Z] += aux_factor * aux_factor1;
  part[m].dipole[Z] += aux_factor * aux_factor1;

  aux_factor1    = gp[n].mass_pos[X] - aux_pos[X]*gp[n].mass;
  part[m].acc[Z] += -3.0 * aux_pos[X] * aux_pos[Z] * dist5 * aux_factor1;
  part[m].dipole[Z] += -3.0 * aux_pos[X] * aux_pos[Z] * dist5 * aux_factor1;

  aux_factor1    = gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass;
  part[m].acc[Z] += -3.0 * aux_pos[Y] * aux_pos[Z] * dist5 * aux_factor1;
  part[m].dipole[Z] += -3.0 * aux_pos[Y] * aux_pos[Z] * dist5 * aux_factor1;
  ...
  aux_factor     = -3.0 * aux_pos[Y] * aux_pos[Z] * aux_factor1;
  part[m].acc[Z] += aux_factor * gp[n].mass_pos[Y];
  */
  return 0;
}//pc_force_mono_dip
...
RETURN: 0
****************************************************************************************************/
int pc_force_mono_dip_quad(int m, int n)
{
  double aux_mass, aux_factor, dist, aux_x, aux_y, aux_z, aux_pos[3];
  double xc, yc, zc;
  ...
  */
  ...
  aux_factor1    = gp[n].mass_pos2[X] - 2.0*aux_pos[X]*gp[n].mass_pos[X] + POW2(aux_pos[X])*gp[n].mass;
  part[m].acc[X] += -0.5 * aux_factor * aux_factor1; 
  part[m].quadrupole[X] += -0.5 * aux_factor * aux_factor1; 

  aux_factor = 15.0 * aux_pos[X] * POW2(aux_pos[Y]) * dist7;
  aux_factor += -3.0 * aux_pos[X] * dist5;
  aux_factor1    = gp[n].mass_pos2[Y] - 2.0*aux_pos[Y]*gp[n].mass_pos[Y] + POW2(aux_pos[Y])*gp[n].mass;
  part[m].acc[X] += -0.5 * aux_factor * aux_factor1;
  part[m].quadrupole[X] += -0.5 * aux_factor * aux_factor1;

  aux_factor = 15.0 * aux_pos[X] * POW2(aux_pos[Z]) * dist7;
  aux_factor += -3.0 * aux_pos[X] * dist5;
  aux_factor1    = gp[n].mass_pos2[Z] - 2.0*aux_pos[Z]*gp[n].mass_pos[Z] + POW2(aux_pos[Z])*gp[n].mass;
  part[m].acc[X] += -0.5 * aux_factor * aux_factor1;
  part[m].quadrupole[X] += -0.5 * aux_factor * aux_factor1;
  
  aux_factor = 15.0 * POW2( aux_pos[X] ) * aux_pos[Y] * dist7;
  aux_factor += -3.0 * aux_pos[Y] * dist5;
  aux_factor1 = gp[n].mass_posxy - aux_pos[X]*gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass_pos[X] + aux_pos[X]*aux_pos[Y]*gp[n].mass;
  part[m].acc[X] += -1.0 * aux_factor * aux_factor1;
  part[m].quadrupole[X] += -1.0 * aux_factor * aux_factor1;
  
  aux_factor1 = gp[n].mass_posyz - aux_pos[Y]*gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass_pos[Y] + aux_pos[Y]*aux_pos[Z]*gp[n].mass;
  part[m].acc[X] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;
  part[m].quadrupole[X] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;

  aux_factor = 15.0 * POW2( aux_pos[X] ) * aux_pos[Z] * dist7;
  aux_factor += -3.0 * aux_pos[Z] * dist5;
  aux_factor1 = gp[n].mass_posxz - aux_pos[X]*gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass_pos[X] + aux_pos[X]*aux_pos[Z]*gp[n].mass;
  part[m].acc[X] += -1.0 * aux_factor * aux_factor1;
  part[m].quadrupole[X] += -1.0 * aux_factor * aux_factor1;
  ...
  */
  ...
  aux_factor     += -3.0 * aux_pos[Y] * dist5;
  aux_factor1    = gp[n].mass_pos2[X] - 2.0*aux_pos[X]*gp[n].mass_pos[X] + POW2(aux_pos[X])*gp[n].mass;
  part[m].acc[Y] += -0.5 * aux_factor * aux_factor1;
  part[m].quadrupole[Y] += -0.5 * aux_factor * aux_factor1;
  ...
  aux_factor += -3.0 * aux_pos[X] * dist5;
  aux_factor1 = gp[n].mass_posxy - aux_pos[X]*gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass_pos[X] + aux_pos[X]*aux_pos[Y]*gp[n].mass;
  part[m].acc[Y] += -1.0 * aux_factor * aux_factor1;
  part[m].quadrupole[Y] += -1.0 * aux_factor * aux_factor1;

  aux_factor = 15.0 * POW2( aux_pos[Y] ) * aux_pos[Z] * dist7;
  aux_factor += -3.0 * aux_pos[Z] * dist5;
  aux_factor1 = gp[n].mass_posyz - aux_pos[Y]*gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass_pos[Y] + aux_pos[Y]*aux_pos[Z]*gp[n].mass;
  part[m].acc[Y] += -1.0 * aux_factor * aux_factor;
  part[m].quadrupole[Y] += -1.0 * aux_factor * aux_factor;

  aux_factor1 = gp[n].mass_posxz - aux_pos[X]*gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass_pos[X] + aux_pos[X]*aux_pos[Z]*gp[n].mass;
  part[m].acc[Y] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;
  part[m].quadrupole[Y] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;
  ...
  */
  ...
  aux_factor += -3.0 * aux_pos[Z] * dist5;
  aux_factor1    = gp[n].mass_pos2[X] - 2.0*aux_pos[X]*gp[n].mass_pos[X] + POW2(aux_pos[X])*gp[n].mass;
  part[m].acc[Z] += -0.5 * aux_factor * aux_factor1;
  part[m].quadrupole[Z] += -0.5 * aux_factor * aux_factor1;
  ...
  aux_factor1    = gp[n].mass_pos2[Z] - 2.0*aux_pos[Y]*gp[n].mass_pos[Z] + POW2(aux_pos[Z])*gp[n].mass;
  part[m].acc[Z] += -0.5 * aux_factor * aux_factor1; 
  part[m].quadrupole[Z] += -0.5 * aux_factor * aux_factor1; 
  
  aux_factor1 = gp[n].mass_posxy - aux_pos[X]*gp[n].mass_pos[Y] - aux_pos[Y]*gp[n].mass_pos[X] + aux_pos[X]*aux_pos[Y]*gp[n].mass;
  part[m].acc[Z] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;
  part[m].quadrupole[Z] += -15.0 * aux_pos[X] * aux_pos[Y] * aux_pos[Z] * dist7 * aux_factor1;

  aux_factor = 15.0 * aux_pos[Y] * POW2(aux_pos[Z]) * dist7;
  aux_factor += -3.0 * aux_pos[Y] * dist5;
  aux_factor1 = gp[n].mass_posyz - aux_pos[Y]*gp[n].mass_pos[Z] - aux_pos[Z]*gp[n].mass_pos[Y] + aux_pos[Y]*aux_pos[Z]*gp[n].mass;
  part[m].acc[Z] += -1.0 * aux_factor * aux_factor1;
  part[m].quadrupole[Z] += -1.0 * aux_factor * aux_factor1;
...
int p3m_forces( int tag )
{
  ...
  if(tag == 0) //Monopole
      ...
      for(m=0; m<GV.NPARTS; m++)
	{
	  ...
	  part[m].quadrupole[X] = 0.0;
	  part[m].quadrupole[Y] = 0.0;
	  part[m].quadrupole[Z] = 0.0;
