...
int main( int argc, char *argv[] )
{
  ...
  for(tag=0; tag<3; tag++)
      ...
      for(i=0; i<GV.NCELLS; i++)
	{
	  for(j=0; j<GV.NCELLS; j++)
		...
		{	      
		  ...
		  /*
		  printf("For cell n=%d, Np=%d, gp.Np=%d\n", 
			 index, Np, gp[index].Np_cell);
		  */
		  ...
		  for(l=0; l<Np; l++)
		    {
		      ...
		      //--- Mass with NGP assignment scheme ---                                                       
		      //Window_fn = W(xc-xp, yc-yp, zc-zp, GV.CellSize[X], GV.CellSize[Y], GV.CellSize[Z]);
		      //gp[index].mass += GV.mass * Window_fn;
		      gp[index].mass += GV.mass;

		      /*
		      printf("For Cell n=%d, xc=%lf, yc=%lf, zc=%lf\n", index, xc, yc, zc);
		      printf("part=%d, xp=%lf, yp=%lf, zp=%lf, cellmass=%lf, w=%lf\n", idPart, xp, yp, zp, gp[index].mass, Window_fn);
		      printf("??????????????\n");
		      fflush(stdout);
		      */
		    }//for l 		  
		  ...
		  if(Np == 0)
		    {
		      printf("Cell n=%d has no particles\n", index);
		      printf("Cell n=%d has mass %lf and should be zero\n", index, gp[index].mass);
		    }
		  */

		  if(gp[index].Np_cell == 0)
		    {
		      gp[index].pos_cm[X] = 0.0;
		      gp[index].pos_cm[Y] = 0.0;
		      gp[index].pos_cm[Z] = 0.0;
		      
		      gp[index].mass_pos[X] = 0.0;
		      gp[index].mass_pos[Y] = 0.0;
		      gp[index].mass_pos[Z] = 0.0;
		      
		      gp[index].mass_pos2[X] = 0.0;
		      gp[index].mass_pos2[Y] = 0.0;
		      gp[index].mass_pos2[Z] = 0.0;
		      
		      gp[index].mass_posxy = 0.0;
		      gp[index].mass_posxz = 0.0;
		      gp[index].mass_posyz = 0.0;
		    }//if
		  else
		    {		      
		      for(l=0; l<Np; l++)
			{		      
			  ...
			  gp[index].pos_cm[X] /= gp[index].mass;
			  gp[index].pos_cm[Y] /= gp[index].mass;
			  gp[index].pos_cm[Z] /= gp[index].mass;
      ...
      for(i=0; i<GV.NTOTALCELLS; i++)
	{
	  ...
	  /*  
	  printf("Cell n=%d, Np=%d, mass=%lf, mpx=%lf, mpy=%lf, mpz=%lf\n", 
		 i, gp[i].Np_cell, gp[i].mass, gp[i].mass_pos[X], gp[i].mass_pos[Y], gp[i].mass_pos[Z]);
	  printf("cm_x = %lf, cm_y=%lf, cm_z=%lf\n", 
		 gp[i].pos_cm[X], gp[i].pos_cm[Y], gp[i].pos_cm[Z]);
	  printf("mpx2=%lf, mpy2=%lf, mpz2=%lf\n", 
		 gp[i].mass_pos2[X], gp[i].mass_pos2[Y], gp[i].mass_pos2[Z]);
	  printf("mpxy=%lf, mpxz=%lf, mpyz=%lf\n",
		 gp[i].mass_posxy, gp[i].mass_posxz, gp[i].mass_posyz );
	  printf("*********************************\n");
	  fflush(stdout);
	  */
	}//for i
      ...
      //snprintf(buffer1, sizeof(char)*100, "./../../Data/Forces_P3M_NParts%ld_NGrid%d_tag%d_CM_correction.bin", GV.NPARTS, GV.NCELLS, tag);
      ...
      for(i=0; i<GV.NPARTS; i++)
	{
	  ...
	  */
      ...
      /*+++++ Verifying dipole and quadrupole values +++++*/
      snprintf(buffer1, sizeof(char)*100, "./../../Data/Dipole_quadrupole_NParts%ld_NGrid%d_tag%d.txt", GV.NPARTS, GV.NCELLS, tag);
      //snprintf(buffer1, sizeof(char)*100, "./../../Data/Dipole_quadrupole_NParts%ld_NGrid%d_tag%d_CM_correction.txt", GV.NPARTS, GV.NCELLS, tag);
      outFile1 = fopen(buffer1, "w");
      
      fprintf(outFile1, "#x\t y\t z\t Dipole_x\t Dipole_y\t Dipole_z\t quadrupole_x\t quadrupole_y\t quadrupole_z\n");
      
      for(i=0; i<GV.NPARTS; i++)
	{
	  fprintf(outFile1,
		  "%16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %16.8lf %16.8lf\n",
		  part[i].pos[X], part[i].pos[Y], part[i].pos[Z], 
		  part[i].dipole[X], part[i].dipole[Y], part[i].dipole[Z],
		  part[i].quadrupole[X], part[i].quadrupole[Y], part[i].quadrupole[Z]);
	}//for i
